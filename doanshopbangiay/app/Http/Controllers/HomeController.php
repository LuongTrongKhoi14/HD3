...
class HomeController extends Controller
{
    public function index(){
        ...
        if(isset($_GET['sort_by'])){
            ...
            if($sort_by =='giam_dan'){
                $all_product = Product::where('product_status','1')->orderBy('product_price', 'DESC')->paginate(30)->appends (request()->query());
            }elseif($sort_by =='tang_dan'){
                $all_product = Product::where('product_status','1')->orderBy('product_price', 'ASC')->paginate(30)->appends (request()->query());
            }elseif($sort_by =='a_z'){
                $all_product = Product::where('product_status','1')->orderBy('product_name', 'ASC')->paginate(30)->appends (request()->query());
            }elseif($sort_by =='z_a'){
                $all_product = Product::where('product_status','1')->orderBy('product_name', 'DESC')->paginate(30)->appends (request()->query());
            }
        }else{
            $all_product = Product::where('product_status','1')->orderBy('product_id','desc')->limit(50)->paginate(6);
        }
    ...
    public function tim_kiem_chitiet_sp(Request $request){
        $keywords = $request->keywords_submit;
        $search_product = Product_details::
        join('tbl_product','tbl_product.product_id','=','tbl_product_details.product_id')
        ...
        ->paginate(15);
        $manager_acc = view('admin.all_product_details')->with('all_product_details',$search_product);
        return view('admin_layout')->with('admin.all_product_details',$manager_acc);
    }
    ...
    public function update_new_pass(){
        ...
        $brand_product =Brand::where('brand_status','1')->orderBy('brand_id','desc')->get();
        $ma_coupon = Coupon::where('coupon_qty','>','1')->orderBy('coupon_id','desc')->get();
        return view('pages.checkout.new_password')->with('category',$cate_product)->with('brand',$brand_product)->with('coupon',$ma_coupon);
    }
